...
def harris_corners(img, window_size=3, k=0.04):
    ...
    for i in range (wby2, H - wby2):
        for j in range (wby2, W - wby2):
            M = np.array([[dxx[i][j],dxy[i][j]],[dxy[i][j],dyy[i][j]]])
            response[i][j] = ((np.linalg.det(M)) - k*((np.trace(M))*(np.trace(M))))
...
def simple_descriptor(patch):
    ...
    if np.std(patch) == 0:
        patch_std = 1
    path_nor = (patch-np.mean(patch))/ patch_std
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(M):
        distance = dists[i, :]
        min_distance = distance[np.argmin(dists[i,:])]
        sec_min_distance = np.sort(distance)[1]
        if min_distance/sec_min_distance <= threshold:
